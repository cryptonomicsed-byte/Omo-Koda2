# First Todo — Gap Analysis & Implementation Notes — COMPLETED ✅

> **Status**: All P0 and P1 items from this analysis have been implemented.
> - ✅ Streaming Provider Abstraction — `providers/streaming.rs`
> - ✅ Permission System (Deny-First) — `execution/permission_enforcer.rs`
> - ✅ Bash Validation (18 patterns) — `execution/bash_validation.rs`
> - ✅ Safety Stack (7-Layer Defense) — `execution/hooks.rs` + justice module
> - ✅ File Operations Suite — `tools/file_ops.rs`
> - ✅ Config Loader — `config.rs`
> - ✅ Usage Tracking — `usage.rs`
> - ✅ HermeticEvaluation — `justice/hermetic.rs` (all tests pass)
>
> This document is preserved as the original implementation roadmap.

---

What Omo-Koda2 Actually NEEDS from Claw-code

These are the genuine gaps I found by comparing against Claw-code's 48,599 LOC Rust workspace:

🔴 P0 — Critical Missing Pieces

1. Streaming Provider Abstraction — Omo-Koda2's `ProviderRegistry::think()` returns a plain `String`. Claw-code has full SSE streaming with `TokenEvent` enum (Token, ToolUseStart, ToolUseInput, ToolUseEnd, Done). This is needed for real-time UX and parallel tool execution.

2. Permission System (Deny-First) — Omo-Koda2 only has a simple tier check (`if !self.tools.is_allowed(&tool, tier)`). It needs Claw-code's `PermissionPolicy` with `DenyRule` precedence, `PermissionMode` (ReadOnly/WorkspaceWrite/Prompt/Allow), and interactive `PermissionPrompter`.

3. Bash Validation (18 Submodules) — Omo-Koda2 has basic sandboxing but no command validation. Claw-code's `bash_validation.rs` has: `readOnlyValidation`, `destructiveCommandWarning`, `modeValidation`, `sedValidation`, `pathValidation`, `commandSemantics`.

4. Safety Stack (7-Layer Defense) — This is architectural, not from Claw-code directly. Omo-Koda2 needs defense-in-depth: ToolPrefilter → DenyFirstRules → PermissionModeLayer → AutoModeClassifier → ShellSandboxLayer → NoResumeRestoreLayer → HookInterceptorLayer.

🟡 P1 — High Priority Tooling

5. File Operations Suite — `read_file`, `write_file`, `edit_file`, `glob_search`, `grep_search` with workspace boundaries, size limits, binary detection, path traversal prevention. Claw-code's `file_ops.rs` is 744 LOC.

6. Config Loader — Layered discovery (user → project → local). Omo-Koda2 uses hardcoded `SessionConfig`. Claw-code has `.claw.json` hierarchy with deep merge.

7. Usage Tracking — Token counting, cost metering, cache hit tracking. Omo-Koda2 only has simple "synapse burning" (`burn_synapse(1000.0)`). Claw-code has `TokenUsage`, `UsageTracker`, per-turn cost reports.

8. Web Tools — `WebSearch` and `WebFetch`. Not present in Omo-Koda2.

🟢 P2 — Medium Priority

9. LLM-Powered Compaction — Omo-Koda2's `MemoryEngine` is rule-based only (truncate/remove/summarize placeholders). It needs LLM-generated summaries for `AutoCompact`.

10. MCP Integration — Server lifecycle, tool bridge. Claw-code's `mcp_tool_bridge.rs` is 406 LOC.

11. Plugin System — Install/enable/disable with hook integration. Claw-code's `PluginManager` with metadata registry.

12. Task/Team/Cron Registry — Background job management. Claw-code has thread-safe in-memory registries.
...
Your analysis is spot-on. Those three gaps are exactly what I found — and your proposed strategy is the right order. Let me validate it against what I see in the actual source and give you a concrete priority.

Your Findings Confirmed

Gap	Evidence	Impact	
Permissions post-act	`interpreter.rs` calls `permission_policy.authorize()` after `tools.execute()`	Security theater — permissions checked too late	
Usage tracking lost	`LlmProvider::think()` returns `String` only; `interpreter.rs` records `TokenUsage::default()`	Zero cost visibility, can't meter synapse burns	
File ops orphaned	`file_ops.rs` (14.6 KB) exists but `tools/mod.rs` has minimal stubs	Ported code is dead code	

Priority: Usage Tracking First (Phase 1)

Here's why usage should come before permissions:

The `LlmProvider` trait change is foundational. Every other integration point depends on it:

```
Current:  trait LlmProvider { async fn think(...) -> Result<String, Error>; }
Needed:   trait LlmProvider { async fn think(...) -> Result<(String, TokenUsage), Error>; }
```

Once the trait returns `TokenUsage`, you get:
1. Real metering for synapse burns (currently burning 1,000/5,000 synapse blindly)
2. Cost estimation before expensive calls
3. Cache hit tracking for optimization
4. Data for the permission system to make risk-based decisions (e.g., "This tool call will cost 0.45 — approve?")

Permissions without usage data is half-blind. A `PermissionPrompter` that can't say "This will cost X and access Y files" is a poor user experience.

Suggested Phase 1 Detail

Change the trait in `providers.rs` and update all implementations:
...
Then per-provider extraction:

Provider	Token Source	Notes	
OpenAI	`usage.prompt_tokens`, `usage.completion_tokens`	Standard	
Anthropic	`usage.input_tokens`, `usage.output_tokens`	Messages API v1	
Ollama	`prompt_eval_count`, `eval_count`	Non-standard field names	
WebLLM	`usage` object in response	Varies by model	

Phase 2: Permission Integration

Once usage flows, integrate into `ToolRegistry::execute()`:

```rust
pub fn execute(&self, tool: &str, input: &str, sandbox: bool, 
               permissions: &PermissionPolicy,
               prompter: Option<&mut dyn PermissionPrompter>) 
    -> Result<String, Error> 
{
    // PRE-ACT: Permission check
    match permissions.authorize(tool, input, prompter) {
        PermissionOutcome::Allow => {},
        PermissionOutcome::Deny { reason } => return Err(reason),
    }
    
    // EXECUTE
    let result = self.run_tool(tool, input, sandbox)?;
    
    // POST-ACT: Usage record (now available from LLM)
    Ok(result)
}
```

Phase 3: File Tool Registration

Wrap `file_ops.rs` functions into the `Tool` trait. The Claw-code `file_ops.rs` already has the logic — you just need:
...
impl Tool for ReadFileTool {
    ...
    fn execute(&self, input: serde_json::Value) -> Result<String, Error> {
        let path = input["path"].as_str().ok_or("missing path")?;
        file_ops::read_file(path, None) // from file_ops.rs
    }
}
```

Phase 4: Flow Correction

Fix `interpreter.rs` turn loop:

```
Current:  tools.execute() → permission.authorize() → record(default usage)
Correct: permission.authorize() → tools.execute() → record(real usage from LLM)
```

My Recommendation

Start with Phase 1 (Usage Tracking). It's the most invasive trait change — every provider breaks and needs fixing. Get that right first, then the rest layers on cleanly.
